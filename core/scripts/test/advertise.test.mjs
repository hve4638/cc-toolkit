...
import { getNextAd } from '../../statusline/lib/advertise/queue.mjs';
import { collectAds, compareSemverDesc, parseAdvertiseTexts } from '../../statusline/lib/advertise/collect.mjs';
import { pickAdText, renderAdLine } from '../../statusline/lib/advertise/render.mjs';
...
function makeConfigDir(root, plugins) {
  ...
  for (const p of plugins) {
    ...
    for (const version of p.versions ?? ['1.0.0']) {
      const versionRoot = join(configDir, 'plugins', 'cache', p.marketplace ?? 'mkt', p.name, version);
      mkdirSync(versionRoot, { recursive: true });
      if (p.advertisable ?? true) writeFileSync(join(versionRoot, '.advertisable'), '');
...
function withTmpDir(fn) {
  const dir = mkdtempSync(join(tmpdir(), 'advertise-test-'));
  return Promise.resolve(fn(dir)).finally(() => rmSync(dir, { recursive: true, force: true }));
}

/** Deps for getNextAd pointing inside the tmp dir. */
function makeDeps(root, configDir, nowMs) {
  return { configDir, globalDir: join(root, 'agent-memory-global'), now: () => nowMs };
}
...
test('collectAds: first scan gathers ads from enabled advertisable plugins', async () => {
  await withTmpDir((root) => {
    ...
    assert.deepEqual(ads.map((a) => a.skill).sort(), ['core:brief', 'core:vt']);
    assert.deepEqual(ads.find((a) => a.skill === 'core:brief').texts, { _: 'One-line brief' });
  });
});
...
test('collectAds: plugins without .advertisable marker are skipped', async () => {
  await withTmpDir((root) => {
    ...
    assert.deepEqual(collectAds(configDir).map((a) => a.skill), ['core:brief']);
  });
});
...
test('collectAds: parses per line, first untagged line becomes the fallback', async () => {
  await withTmpDir((root) => {
    const configDir = makeConfigDir(root, [
      { name: 'core', skills: { multi: '\n\n  First line here\nsecond line ignored\n', blank: '   \n\t\n' } },
    ]);
    assert.deepEqual(collectAds(configDir), [{ skill: 'core:multi', texts: { _: 'First line here' } }]);
  });
});
...
test('parseAdvertiseTexts: language tags, fallback, and false-positive guard', () => {
  ...
  assert.deepEqual(parseAdvertiseTexts('en: First\nen: Second'), { en: 'First' });
  ...
  assert.deepEqual(parseAdvertiseTexts('verylongword: not a tag'), { _: 'verylongword: not a tag' });
  ...
  assert.deepEqual(parseAdvertiseTexts('en: \x1b[31mRed\x1b[0m alert\x07 text'), { en: '[31mRed[0m alert text' });
  ...
  assert.deepEqual(parseAdvertiseTexts('\x07Note: still not a tag'), { _: 'Note: still not a tag' });
});

test('pickAdText: prototype keys in the language resolve nothing inherited', () => {
...
test('pickAdText: selection order lang → fallback → en → first entry', () => {
  ...
  assert.equal(pickAdText({ skill: 's', texts: { fr: 'français', en: 'english', _: 'neutral' } }, 'ko'), 'neutral');
  assert.equal(pickAdText({ skill: 's', texts: { fr: 'français', en: 'english' } }, 'ko'), 'english');
...
test('collectAds: control characters are stripped from ad text', async () => {
  await withTmpDir((root) => {
    const configDir = makeConfigDir(root, [
      { name: 'core', skills: { sneaky: '\x1b[31mRed\x1b[0m alert\x07 text\r', onlyctl: '\x07\x1b\x00 \x08' } },
    ]);
    assert.deepEqual(collectAds(configDir), [{ skill: 'core:sneaky', texts: { _: '[31mRed[0m alert text' } }]);
  });
});

test('collectAds: only the latest version dir is scanned', async () => {
  await withTmpDir((root) => {
    const configDir = makeConfigDir(root, [{ name: 'core', versions: ['0.9.0'], skills: { old: 'Old ad' } }]);
    ...
    assert.deepEqual(collectAds(configDir), [{ skill: 'core:fresh', texts: { _: 'New ad' } }]);
  });
});
...
test('first call scans, writes cache and queue, returns an ad', async () => {
  await withTmpDir((root) => {
    const configDir = makeConfigDir(root, [{ name: 'core', skills: { brief: 'One-line brief' } }]);
...
test('same date: no rescan even when plugins changed on disk', async () => {
  await withTmpDir((root) => {
    ...
    assert.deepEqual(getNextAd({ ...deps, now: () => DAY1_NOON + 2 * 60_000 }).texts, { _: 'One-line brief' });
  });
});
...
test('date change: rescans and rebuilds the queue', async () => {
  await withTmpDir((root) => {
    ...
    assert.equal(readJson(join(deps2.globalDir, 'advertise-cache.json')).date, '2026-01-02');
  });
});

test('throttle: within 1 minute the current ad is returned without a write', async () => {
  await withTmpDir((root) => {
    const configDir = makeConfigDir(root, [{ name: 'core', skills: { a: 'Ad A', b: 'Ad B' } }]);
    const deps = makeDeps(root, configDir, DAY1_NOON);
    ...
    assert.deepEqual(getNextAd({ ...deps, now: () => DAY1_NOON + 59_000 }), first);
    assert.equal(readFileSync(queuePath, 'utf-8'), before);

    assert.notDeepEqual(getNextAd({ ...deps, now: () => DAY1_NOON + 61_000 }), first);
  });
});
...
test('zero ads: cache is still written (no same-day rescan), returns null', async () => {
  await withTmpDir((root) => {
    const configDir = makeConfigDir(root, [{ name: 'quiet', advertisable: false, skills: { x: 'Hidden' } }]);
    const deps = makeDeps(root, configDir, DAY1_NOON);
    assert.equal(getNextAd(deps), null);
    assert.deepEqual(readJson(join(deps.globalDir, 'advertise-cache.json')).ads, []);
...
test('future consumedAt does not lock the throttle forever', async () => {
  await withTmpDir((root) => {
    ...
    assert.ok(getNextAd({ ...deps, now: () => DAY1_NOON + 1000 }));
...
test('corrupt queue file is silently rebuilt from the cache', async () => {
  await withTmpDir((root) => {
    ...
    assert.deepEqual(getNextAd({ ...deps, now: () => DAY1_NOON + 2 * 60_000 }), {
      skill: 'core:brief',
...
test('old-schema cache rescans once even on the same date', async () => {
  await withTmpDir((root) => {
    const configDir = makeConfigDir(root, [{ name: 'core', skills: { brief: 'One-line brief' } }]);
    const deps = makeDeps(root, configDir, DAY1_NOON);
    mkdirSync(deps.globalDir, { recursive: true });
    const cachePath = join(deps.globalDir, 'advertise-cache.json');
    writeFileSync(cachePath, JSON.stringify({ date: '2026-01-01', ads: [{ skill: 'core:old', text: 'Old schema ad' }] }));
    assert.deepEqual(getNextAd(deps), { skill: 'core:brief', texts: { _: 'One-line brief' } });
...
test('old-schema queue items are dropped and the queue rebuilds from the cache', async () => {
  await withTmpDir((root) => {
    ...
    getNextAd(deps);
    writeFileSync(
      join(deps.globalDir, 'advertise-queue.json'),
      JSON.stringify({
    ...
    assert.deepEqual(getNextAd({ ...deps, now: () => DAY1_NOON + 1000 }), {
      skill: 'core:brief',
      texts: { _: 'One-line brief' },
    });
  });
...
test('renderAdLine: dim framing around a bright blue skill reference', () => {
  const ad = { skill: 'core:brief', texts: { _: 'One-line brief' } };
  assert.equal(
    renderAdLine(ad, 'en'),
    '\x1b[2mtry \x1b[0m\x1b[94m/core:brief\x1b[0m\x1b[2m [One-line brief]\x1b[0m',
  );
  assert.equal(renderAdLine({ skill: 'core:brief', texts: {} }, 'en'), null);
});
