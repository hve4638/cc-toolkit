...
현재 Claude Code 세션의 transcript(`.jsonl`)를 정제·조회·추출하는 도구. 결정론적 CLI(`ccrec`)가 엔진이고, 사람이 직접 또는 에이전트가 자연어로 호출한다.
...
- 진입점명 `ccrec`.
- `core/bin/ccrec` — 1급 CLI. core가 `<루트>/bin`을 PATH에 자동 노출하므로 어디서나 `ccrec …`.
- skill 폴더(`core/skills/ccrec/`)에 엔진(`scripts/ccrec.mjs`)을 두고, bin은 그것을 부르는 얇은 런처. `node` 외 추가 의존성 0.
...
- 정제본을 `~/.cache/ccrec/<session>.slim.jsonl`에 자동 관리(`XDG_CACHE_HOME` 존중).
...
```
ccrec [--session <id> | --source <jsonl>] <command> [args]
```
...
- 변환기(`clip`/`filter`): `jsonl→jsonl`. stdin이 파이프로 오면 그걸, 아니면 캐시를 입력.
  - `ccrec clip 4 8 | ccrec filter --tool Bash`
...
- ccrec(결정론): 파싱·슬림화·캐시·count·search·clip·filter.
