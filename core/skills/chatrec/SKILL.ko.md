...
현재 세션 transcript(디스크의 `.jsonl` 로그)를 `chatrec` CLI로 조회한다. 구간을 찾고, 그 구간만 추출한다 — transcript 전체를 컨텍스트에 올리지 않는다.

컨텍스트 기억이 아니라 chatrec 출력에서 작업한다. 긴 세션은 요약됐을 수 있어 기억은 손실되지만, transcript 파일은 전체 기록이다.

## 호출

`chatrec`는 PATH에 있다(core가 `bin/`을 노출). 못 찾으면 다음으로 대체:
...
| 명령 | 출력 |
|---|---|
| `chatrec count` | 총 턴 수 + role 분포 |
| `chatrec search [pat] [filters]` | 매칭 턴 → `T<t>  <role>  <스니펫>` |
| `chatrec clip <from> [to]` | 그 턴 구간의 레코드 (jsonl) |
| `chatrec filter [filters]` | 거른 레코드 (jsonl) |

필터(search·filter): `--role user,assistant,tool_call,tool_result` · `--tool Bash,Read` · `--match "문구"` · `--from N --to M` · `--invert`.

`clip`/`filter`는 jsonl→jsonl이라 파이프로 잇는다: `chatrec clip 4 8 | chatrec filter --tool Bash`. `--out <파일>`을 붙이면 stdout 대신 파일로 쓴다.

## 워크플로우

1. `chatrec count` — 규모 파악.
2. 요청을 턴에 대응: `chatrec search "X"`(또는 `--tool` / `--role`) → 매칭 `T<t>`.
3. 그 구간만 추출: `chatrec clip <from> <to>`, 필요하면 `filter`/파이프로 좁힘.
4. 산출물 생성:
