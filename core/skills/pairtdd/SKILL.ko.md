...
전체 스위트 테스트 커맨드를 파악하고 실제로 실행되는지 확인한다. 프로젝트에 테스트 러너가 아직 없으면 먼저 셋업해서 커밋한다. 커맨드를 스펙 파일에 `test-cmd: <command>` 줄로 기록한다 — 양쪽이 같은 커맨드를 쓴다.

## Step 3: tester 스폰

tester 하나만 named subagent 로 띄운다:

- `Agent({name: "tdd-adversary", subagent_type: "tdd-adversary", prompt: "작업 디렉터리는 <repo root>. 다음 SendMessage 로 도착할 bootstrap 메시지까지 대기."})`

named agent 는 논블로킹으로 mailbox 모드에 뜬다. 턴 결과는 Agent 도구 결과로 돌아오지 않는다 — tester 의 모든 신호는 `tdd-adversary` 가 보내는 SendMessage 로 도착한다.
...
`no_progress_count` 를 0 으로 시작. tester 의 각 신호는 `tdd-adversary` 가 보내는 메시지로 도착한다; 받아서 처리한다. 신호 형식이 아닌 메시지 (예: `idle_notification` JSON) 는 무시한다 — 아래 신호 형식만 루프를 움직인다:

- `<sha>: <case>` (새 red 커밋) → `no_progress_count = 0`. main 이 이번 라운드 구현자다:
  1. `git show <sha>` 로 실패 테스트를 읽는다.
  2. 테스트가 스펙에 없는 동작을 요구하면 구현하지 않는다 — `SendMessage(to: "tdd-adversary", message: "dispute: <스펙 인용 포함 사유>")` 를 보내고 아래 dispute 신호 분기로 처리한다.
  3. 아니면 프로덕션 코드를 작성해 통과시킨다. 평소 main 세션 흐름·inlay·외부 규칙을 그대로 따른다.
  4. 전체 테스트 스위트를 실제로 실행해 (스펙의 `test-cmd`) **전부 green 인지 확인한다** (가정 금지).
  5. 커밋한다. 중간 커밋 개수·메시지 형식은 자유.
  6. `git rev-parse HEAD` 로 최종 green 커밋 SHA 를 얻어 `SendMessage(to: "tdd-adversary", message: "last-impl-sha=<sha>")`.
...
- `dispute-accepted: reverted <revert-sha>` → tester 가 테스트를 철회했다. `retry: produce the next red` 를 보내고 루프를 계속한다.
- `dispute-upheld: <spec quote>` → 구현한다 — red 커밋 분기의 3–6 단계 (green 확인, 커밋, `last-impl-sha` 송신) 로 이어간다. 인용을 보고도 여전히 스펙 밖이라고 판단하면 양쪽 입장을 사용자에게 올려 판정을 따른다.
...
수렴 또는 사용자 중단 시:
- git 히스토리로 요약한다: `git log --oneline` 으로 라운드 타임라인을, `tdd(red):` 커밋들의 `[spec: <clause>]` 태그를 스펙 조항과 대조해 커버된·안 된 조항을 보고한다.
- tester 는 그대로 둔다; 종료 메시지가 필요 없다.

## 가드레일 (코어 — 보존된 tester 가 의존하므로 불변)

- tester 의 테스트 파일을 수정·삭제하지 않는다. main 은 프로덕션 코드만 건드린다. 테스트를 약화시키면 tester 가 무의미해진다 — 그리고 tester 가 매 라운드 구현자 커밋의 테스트 파일 변경을 감사한다.
- tester 에게 턴을 넘기기 전, HEAD 는 **전체 스위트를 실제 실행해 green 인** 커밋이고, working tree 는 clean 이다.
- 스펙은 `.agent-memory/tdd-spec/<slug>.md` 에 두고 tester 와 공유한다. bootstrap 이후 동결된다 — dispute 의 판정 기준이므로, 변경은 사용자 승인 경유.
