---
name: ticket
description: "현재 세션에서 드러난 플러그인(skill/hook) 문제를 분석해 봉인된 피드백 티켓으로 보관한다"
...
현재 세션에서 드러난 플러그인 문제를 분석해 봉인된 피드백 티켓으로 보관한다. 아카이브는 자신의 sha256으로 키를 잡으므로, 한 번 봉인하면 불변이다.
...
```bash
node "${CLAUDE_PLUGIN_ROOT}/skills/ticket/scripts/seal-ticket.mjs" <스테이징-디렉토리>
```

tar.gz를 만들고, sha256을 계산하고, `~/.agent-memory/global/ticket/<sha256>.tar.gz`로 옮기고, `~/.agent-memory/global/ticket/index.jsonl`에 메타 한 줄을 append하고, 최종 경로와 해시를 출력한다. 그 출력을 사용자에게 보고한다.

## report.md frontmatter (고정 필드)

```yaml
id:               # 예: tk-2026-06-30-001 (날짜 기반, 직접 부여)
