...
로컬 Claude Code 히스토리에 대한 사용량 질문을 `tracetranscript` CLI 로 답한다. `~/.claude/projects` 아래 모든 transcript 를 스캔한다 (서브에이전트 transcript 는 부모 세션에 합산, 토큰 사용량은 API 호출 단위로 중복 제거).

## 호출

`tracetranscript` 는 PATH 에 있다 (core 가 `bin/` 을 노출). 없으면 다음으로 대체:
...
1. 스캔은 scratchpad/tmp 에 한 번만: `tracetranscript scan --since 1m --out /tmp/…/scan.jsonl`.
