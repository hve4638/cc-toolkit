---
name: ut
description: "[useterminal] 사용자의 tmux 창에 pane 을 열고 조작해 무언가를 직접 보게 한다. 사용자가 데모를 요청하거나 프로그램이 도는 것을 보고 싶어 할 때, 또는 출력이 사용자가 볼 수 있는 자리에 놓여야 할 때 사용한다."
...
- 단순 설명의 경우. 색상 코드를 적극적으로 활용하여 가독성을 높여라.
- 여러 작업을 순차적으로 보여줄 때 clear를 남발해서는 안된다. 사용자가 확인하지 못하고 지나가게 된다. 연관성 없는 새로운 작업 시연을 요청 받을시 한번한 clear하는 정도로만 사용할 것.
- `... | less` 와 같이 파이프라인을 이용하는 경우 출력 결과에서 색상이 사라져 가독성이 하락하는 문제가 있다.
- 두 출력을 비교하거나 여러 케이스를 보여줄 때는 2~3개의 pane을 통해 보여주는 것이 권장된다.
- 사용자가 볼 수 있는 최적의 pane 개수는 3개이며, 4개까지는 10줄 이내의 출력 표시 용도로는 허용된다. 그 이상의 pane을 쓰는 것은 권장하지 않는다.
</useterminal_instruction>
