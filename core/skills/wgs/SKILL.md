---
name: wgs
description: "[writing-great-skill] Reference for writing, editing, and reviewing skills — the vocabulary and principles that make a skill predictable. Pass file paths to review them against it."
...
- **Respect the format limits.** Third person, what + when only, under 1024 characters, no XML tags. A behavioural rule living in the description moves to the body.
...
Disclosure has mechanical bounds: the chain stays one hop deep — a file linked from `SKILL.md` does not link onward — and a reference file past ~100 lines opens with a table of contents. Deterministic work (fixed computation, transformation, templates) moves out of prose entirely, into `scripts/` and `assets/` the body invokes, leaving prose only where judgement is needed. And a body past ~500 lines is **sprawl** however live each line is.
...
- A prohibition that can be recast as transparency — "if X was done, declare it" — takes that form; it steers structurally where **negation** steers badly.
