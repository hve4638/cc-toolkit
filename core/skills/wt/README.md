...
- `.wtrc` 에 `post_create` 훅이 있으면 그걸 부른다. `/wt` 가 기본으로 써주는 훅은 tmux 윈도우를 여는 것이고(창을 열지, 거기서 claude 를 띄울지는 `/wt` 셋업 때 사용자에게 물어 정한다), 다른 mux 든 알림이든 그 자리를 바꾸면 된다.
...
- **디렉토리(슬러그)**는 그 브랜치명을 파일시스템용으로 변환 — `/` → `-` (예: `feat/login` → 워크트리 폴더 `feat-login`). 슬러그 변환은 폴더명에만 적용되고 브랜치명은 그대로 쓴다(예시 훅의 tmux 윈도우명은 `win_prefix` + 브랜치명).
...
| `win_prefix` | 예시 훅이 여는 tmux 윈도우명 접두어. 비면 브랜치명 그대로 |
...
> **`WT_INTERACTIVE` 가 `[ -t 1 ]` 가드를 대체한다.** 예전엔 mkwt 가 "터미널이면 tmux 창, 아니면 경로 출력" 을 스스로 정했다. 지금은 사용자가 앞에 있는지만 알려주고 판단은 훅이 한다 — 창 열기는 `/wt` 셋업 때 정한 대로, 알림은 언제나처럼 **동작마다 답이 다를 수 있기 때문**이다. mkwt 가 대신 정하면 후자가 불가능해진다.
...
`.wtrc` 가 source 되므로 이름이 mkwt 와 같은 공간에 놓인다. 그래서 mkwt 내부는 전부 `mkwt_` 접두어를 쓰고, 접두어 없는 이름(`repo`, `worktree_dir`, `post_create`)만이 계약이다. 템플릿의 `win_prefix` 는 예시 훅만 읽는 값으로, mkwt 자신은 모른다.
