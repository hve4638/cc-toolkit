...
# Same normalization as the old wt-new: strip control/git-forbidden chars, fold
# whitespace and slashes to '-', keep other text (incl. non-ASCII) as is.
...
case "$slug" in
  @|HEAD) die "name '$raw_name' makes a reserved ref name ('$slug')" ;;
esac
...
if ! git -C "$repo" worktree add -b "$slug" "$target" HEAD 1>&2; then
...
# worktree, named after the slug. Otherwise — piped/captured/redirected, e.g. an
...
if [ -n "${TMUX:-}" ] && [ -t 1 ] && tmux new-window -c "$target" -n "$slug" 2>/dev/null; then
