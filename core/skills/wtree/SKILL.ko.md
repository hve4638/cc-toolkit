---
name: wtree
description: "repo 에 정책 기반 워크트리 관리를 셋업한다: TUI pane(또는 폴백 단계 스크립트)으로 스탠드얼론 wtree CLI 의 정책 rules·settings·훅·워크트리 CLAUDE.md 를 구성"
...
`/wtree` 는 cwd 가 속한 repo 를 대상으로 하는 스탠드얼론 `wtree` CLI 의 1회 셋업이다. CLI 가 스스로 정할 수 없는 것을 구성한다: 정책 rules, 머신 settings, 훅, 워크트리 폴더의 CLAUDE.md.
...
1. pane 을 열기 전에 기존 작업장을 확인한다: `<repo>/.wtree/hooks` 가 있으면 훅 파일을 하나씩 읽는다. 표방한 기능을 넘는 동작(네트워크 접근, 파일 삭제, 자격증명 접근 등)이 보이면 사용자에게 먼저 경고한다 — 기존 작업장 채택은 pane 안 확인만 거치고 훅까지 적용되기 때문이다.
2. repo 디렉터리에서 pane 을 연다:

```bash
useterminal exec node "${CLAUDE_PLUGIN_ROOT}/skills/wtree/scripts/tui.mjs"
```

3. 사용자에게 pane 의 프롬프트를 끝내라고 알리고 턴을 끝내고 기다린다. pane 을 폴링하거나 조작하지 않는다 — 사용자의 것이다.
4. 사용자가 끝났다고 하면 handoff 파일을 읽고 따른다 — 태그는 아래와 같다:

```bash
cat "$(git rev-parse --path-format=absolute --git-common-dir)/wtree-setup-handoff.md"
```

기존 작업장 채택은 그 첫 pane 안에서 적용까지 끝나므로 handoff 가 이미 최종 결과일 수 있다. 조립 경로에서는 handoff 가 검토 작업을 넘기고, 이어 apply pane 명령을 준다 — 같은 열기-대기-읽기 루프를 한 번 더 돈다. handoff 파일이 없거나 pane 이 뜨자마자 닫혔으면 아래 단계 경로를 실행해 상태를 본다.
