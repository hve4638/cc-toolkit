...
<alert>
이 .wtree 는 훅을 포함한다: {HOOK_FILES}

사용자가 그대로 적용을 고르기 전에 훅 파일을 각각 읽는다. 선언된 기능 밖의 동작(네트워크 접근, 파일 삭제, 자격증명 접근 등)이 보이면 사용자에게 경고를 앞세운다. 훅이 하는 일을 설명하고 훅도 복사할지 묻는다 — 그 답이 copy_hooks 값이 된다.
</alert>
<require>
- path: "{DOTWTREE}" | "/tmp/<새 폴더>"
- allow_overwrite: true — 폐기 재구성일 때만
- copy_hooks: true | false — 그대로 적용일 때만, 검토 후 결정
</require>
<question>
1. 기존 정책의 처분 (select one)
- 그대로 적용: 발견된 정책을 이 repo 에 그대로 쓴다 (path: "{DOTWTREE}", copy_hooks: true|false)
