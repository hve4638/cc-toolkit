...
'use strict';
/**
 * frame — cross-platform hook runner (run.cjs)
 *
 * Spawns the target .mjs hook with process.execPath (the Node binary already
 * running this script), bypassing PATH / shell discovery so nvm/fnm and Windows
 * users all hit the right binary. Resolves stale CLAUDE_PLUGIN_ROOT paths left
 * by a plugin update, and applies the per-hook timeout declared in hooks.json.
 *
 * Usage (from hooks.json):
 *   node "${CLAUDE_PLUGIN_ROOT}/scripts/run.cjs" \
 *       "${CLAUDE_PLUGIN_ROOT}/scripts/dispatch.mjs" <EventName>
 *
 * Vendored from core/scripts/run.cjs.
 */
