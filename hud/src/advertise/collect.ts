 ...
 */
export function parseAdvertiseTexts(raw: string): Record<string, string> {
 ...
 */
function collectPluginAds(pluginName: string, versionRoot: string): AdItem[] {
  const ads: AdItem[] = [];
 ...
 */
export function collectAds(configDir: string): AdItem[] {
  const ads: AdItem[] = [];
