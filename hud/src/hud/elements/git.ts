...
const localRepoCache = new Map<string, CacheEntry<string | null>>();
 ...
 */
export function getLocalRepoName(cwd?: string): string | null {
  const key = cwd ? resolve(cwd) : process.cwd();
 ...
 * @returns Repository name or null if not in a git repo
 */
export function getRepoDisplayName(cwd?: string): string | null {
