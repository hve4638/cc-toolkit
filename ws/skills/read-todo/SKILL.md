...
1. `wstodo [경로]` 를 실행해 전체 표를 얻는다. 경로를 생략하면 현재 위치에서 워크스페이스 루트 방향으로 docs/todo 를 찾는다. 이름을 인자로 주면 (`wstodo <milestone명|todo명>`) 그 대상과 선행 todo 만 좁혀 보여주고, `--detail` 을 붙이면 milestone 소속 `[이름]` 과 간접 선행 `(이름)` 표식이 함께 나온다.
2. 요청을 표에 대어 대상을 좁힌다:
   - 특정 todo 명 → 그 폴더의 TODO.md 와 딸린 자료를 읽는다. 선행 관계 질문 ("이거 하려면 뭐부터") 이면 `wstodo <todo명>` 으로 좁힌다
   ...
   - milestone 질문 (진행률·남은 범위) → `wstodo <milestone명>` 을 실행한다. done (완료) · open (진행 중) · missing (열거만 되고 미생성) 요약과 선행 순서가 나온다
