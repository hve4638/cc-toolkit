...
- 완료: `wstodo <milestone명>` 을 실행해 완료 성립 (열거된 모든 이름이 DONE.md 에 있음) 을 확인한 뒤 폴더째 삭제한다. 성립하지 않으면 출력에 남는 open·missing 이름을 알리고 멈춘다.

변형 후 `wstodo [경로]` 를 실행해 경고 없이 파싱되는지 확인하고, 각 규칙 문서의 수정 규칙대로 ws repo 에 커밋한다.
</write-todo>
